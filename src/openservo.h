...
struct sv
{
  bool responsive;  // TODO: not set at the moment
  bool updated;
  // read only (11 values)
...
public:
  ...
  //int setCurvePosition(int servo_id, int value);
  int setCurveInVelocity(int servo_id, int value);
  int setCurveOutVelocity(int servo_id, int value);

  int setAddress(int servo_id, int address); // change servo message bus ID
...
private:
  ...
  int sendData(unsigned char addr, unsigned char data_addr, unsigned char* data, int data_len); // write data on device
